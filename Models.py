...
def get_layer(type_l='linear', inputs=28, outputs=10, drop=0.5, in_c=1, out_c=10,ker_size=[5,2], srides=[1, 2]):
    ...
    :param srides: the strides of convolution and polling
    :return: the layer
    '''
    ...
    if (type_l== 'conv'):
        return [
            ...
            #nn.GELU(),
            #nn.MaxPool1d(kernel_size=ker_size[1], stride=srides[1])
        ]
    if (type_l== 'conv1D'):
        return [
            nn.Conv1d(in_c, out_c,
    ...
    if (type_l == 'conv1D_bn_bloc'):
        ...
        bloc.append(nn.MaxPool1d(kernel_size=ker_size[1], stride=srides[1]))
...
class CNN(nn.Module):
    ...
    def __init__(self, inc=1, xr_size=200,xr=False, out=3,out_src=0, ker_size=[3,2],
        ...
        :param inc: nb input channels
        :param xr_size: second input size
        :param out: output size
        :param ker_size: size of kernels
        :param strides: the strides of convolution and polling
        :param nb_filtres:
        :param Dr: the drop out proba
        :param FC_L_size: size of each fully connected layer
        :param l2reg: L2 regulation value
        '''
        ...
        self.conv_classifier = get_layer(type_l='conv', in_c=nb_input, out_c=nb_input * nb_filtres[0],
                                         ker_size=ker_size, srides=strides)
        ...
        #fc_input = nb_input * nb_filtres[-1] + xr_size
        #fc_input = 500 #+ xr_size
...
class CNN_residual(nn.Module):
    ...
    def __init__(self, inc=1,xr_size=200,out=3,out_src=0,ker_size=[5,2],
        ...
        self.Conv_layers=[]
        self.conv_classifier=get_layer(type_l='conv2',in_c=nb_input,out_c=nb_input*nb_filtres[0],ker_size=ker_size,srides=srides)
        self.Conv_layers.extend(self.conv_classifier)
        self.Weight.append(self.conv_classifier[0])
        for i in range(1,len(nb_filtres)):
            nb_input*=nb_filtres[i-1]
            self.conv_classifier = get_layer(type_l='conv2', in_c=nb_input, out_c=nb_input*nb_filtres[i],ker_size=ker_size,srides=srides)
            self.Conv_layers.extend(self.conv_classifier)
            self.Weight.append(self.conv_classifier[0])
...
class RNN(nn.Module):
    ...
    def __init__(self,out=3,out_src=0):
        ...
        self.rnn = nn.LSTM(bidirectional=True, num_layers=2,dropout=0.5, input_size=768,hidden_size=200, batch_first=True)
        self.dropout = torch.nn.Dropout(0.2)
        #self.fc = Fully_Connected(input=400,out=out)
...
class Bert_finetuning_CNN(nn.Module):
    def __init__(self, out=3,out_src=0, bert_type='bert'):
        ...
        _,self.bert_model=bert.get_bert(bert_type=bert_type)
        self.cnn = CNN(inc=768,Dr=[0.5,0.5],nb_filtres=[8],FC_L_size=[768],ker_size=[3,2],padding=False,maxpooling_dir=True, out=out,out_src=out_src)
...
class Bert_finetuning_CNN_resedual(nn.Module):
    def __init__(self, out=3,out_src=0, bert_type='bert'):
        super(Bert_finetuning_CNN_resedual, self).__init__()
        _,self.bert_model=bert.get_bert(bert_type=bert_type)
        self.cnn = CNN(inc=768,nb_filtres=[3],FC_L_size=[768],ker_size=[3,2],padding=False,maxpooling_dir=True, out=out,out_src=0,w_size=768)

    def forward(self,x,xr):
        activity_layers, _ = self.bert_model(x)
        #if not isinstance(activity_layers, torch.Tensor):
        #    x1 = activity_layers[-1]
        #x1=x1.permute(0,2,1)
...
class Bert_finetuning_CNN2(nn.Module):
    def __init__(self, out=3,out_src=0, bert_type='bert'):
        ...
        self.cnn1 = CNN(inc=768,Dr=[0.5,0.5],nb_filtres=[8],FC_L_size=[768],ker_size=[3,2],padding=False,maxpooling_dir=True, out=100,out_src=out_src)
        self.cnn2 = CNN(inc=768, Dr=[0.5,0.5], nb_filtres=[8], FC_L_size=[768], ker_size=[5,2], padding=False,maxpooling_dir=True, out=100, out_src=out_src)
    def forward(self,x,xr):
        activity_layers, _ = self.bert_model(x)
        if not isinstance(activity_layers, torch.Tensor):
            activity_layers = activity_layers[-1]
